...
static inline int to_chars(const floating_decimal_64 v, const bool sign, char* const result) {
  ...
  const int32_t k = v.exponent;
  const int32_t kk = (int32_t)olength + k;
  if (-4 < kk && kk <= 8) {
    if (olength > 1) {
      memmove(result + index + 1, result + index + 2, olength - 1);
    }
    if (k >= 0) {
      index += (int32_t)olength;
    ...
    if (kk > 0) {
      memmove(result + index + kk + 1, result + index + kk,
              (size_t)((int32_t)olength - kk));
      result[index + kk] = '.';
      return index + (int32_t)olength + 1;
    }
    const int32_t offset = 2 - kk;
    memmove(result + index + offset, result + index, olength);
    result[index] = '0';
    result[index + 1] = '.';
    for (int32_t j = 2; j < offset; j++) {
      result[index + j] = '0';
    }
    return index + offset + (int32_t)olength;
  }
